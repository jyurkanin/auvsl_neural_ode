...
void TireNetwork::load_model(){
 ...
 Eigen::Matrix<float,num_hidden_nodes,num_in_features> weight0;
 Eigen::Matrix<float,num_hidden_nodes,1> bias0;
 Eigen::Matrix<float,num_hidden_nodes,num_hidden_nodes> weight2;
 Eigen::Matrix<float,num_hidden_nodes,1> bias2;
 Eigen::Matrix<float,num_out_features,num_hidden_nodes> weight4;
 Eigen::Matrix<float,num_out_features,1> bias4;

 Eigen::Matrix<float,num_out_features,1> out_mean;
 Eigen::Matrix<float,num_out_features,1> out_std;
 Eigen::Matrix<float,num_in_features,1> in_mean;
 Eigen::Matrix<float,num_in_features,1> in_std_inv; //inverse of in_std. Because multiply is faster than divide.
...
in_std_inv = in_std_inv.cwiseInverse();
 ...
 for(int i = 0; i < m_weight0.rows(); i++)
 {
   ...
   {
     m_weight0(i,j) = Scalar(weight0(i,j));
   }
 }
 for(int i = 0; i < m_weight2.rows(); i++)
 {
   ...
   {
     m_weight2(i,j) = Scalar(weight2(i,j));
   }
 }
 for(int i = 0; i < m_weight4.rows(); i++)
 {
   ...
   {
     m_weight4(i,j) = Scalar(weight4(i,j));
   }
 }

 for(int j = 0; j < m_bias0.size(); j++)
 {
   m_bias0[j] = Scalar(bias0[j]);
 }
 for(int j = 0; j < m_bias2.size(); j++)
 {
   m_bias2[j] = Scalar(bias2[j]);
 }
 for(int j = 0; j < m_bias4.size(); j++)
 {
   m_bias4[j] = Scalar(bias4[j]);
 }

 for(int j = 0; j < m_out_mean.size(); j++)
 {
   m_out_mean[j] = Scalar(out_mean[j]);
 }
 for(int j = 0; j < m_out_std.size(); j++)
 {
   m_out_std[j] = Scalar(out_std[j]);
 }
 
 for(int j = 0; j < m_in_mean.size(); j++)
 {
   m_in_mean[j] = Scalar(in_mean[j]);
 }
 for(int j = 0; j < m_in_std_inv.size(); j++)
 {
   m_in_std_inv[j] = Scalar(in_std_inv[j]);
 }
