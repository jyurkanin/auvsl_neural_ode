...
void BekkerDynamics::get_tire_f_ext(const Eigen::Matrix<Scalar,STATE_DIM,1> &X, LinkDataMap<Force> &ext_forces)
{
	...
	for(int ii = 0; ii < 4; ii++){    
		...
		inputs[0] = sinkages[ii];
		inputs[1] = tire_vx;
		inputs[2] = cpt_vels[ii][1];
		inputs[3] = vel_x_tan;
		
		bekker_features = m_bekker_tire_model.get_features(inputs);
		...
		forces[0] = CppAD::CondExpGt(vel_x_tan - cpt_vels[ii][0], ADF(0.0), CppAD::abs(forces[0]), -CppAD::abs(forces[0]));
