...
** DONE Not sure where my pretraining script went.
The pretraining script doesn't match the C++ implementation
In terms of the tire network formulation. Which makes no sense.

** TODO Add logic to set all reaction forces to zero during testing
During training, allow this nonsense to happen for the purpose of creating
a smooth simulation.
During testing, make it realistic by setting these reaction forces to zero.


* <2023-09-25 mon 23:40>
plots you want

** plot all models on the 144 different cv3 trajectories
Nah... Maybe just as a unit test
...
** DONE plots for example trajectories cv3#55, cv3#1, and a mostly straight CV3 would be good
...
Things I don't want to do but might have to do:

** Replace Scalar and ADF with float
For a speed test. Super annoying to do this and I really don't
want to do this.

** Train on the other non-off-road datasets just to show it can be done
Takes time and effort I don't feel like giving.

** Train on the other datasets all with the same model, to show the model can learn to distinguish different ground types.
I would have to append a 3 element 1 hot vector to the TireModel's
input vector, depending on the type of dataset and retrain. It would be
sort of painful to do this and I really don't want to.
