...
Current validation loss (at 10 steps) is .004 so that might bode
well for 60 steps. Strange though, because the validation loss is
represented as a ratio of total distance travelled so I would
expect it to be higher for small trajectories. Oh, this branch
didn't have that. Great.
