...
def print_c_network(md, in_std, output_scaler):
...
class TireNet(nn.Module):
  def __init__(self):
    ...
    self.model_z = nn.Sequential(
      nn.Linear(1, self.hidden_size2, bias=False),
      nn.Tanh(),
      nn.Linear(self.hidden_size2, self.hidden_size2, bias=False),
      nn.Tanh(),
      nn.Linear(self.hidden_size2, 1, bias=False),
      nn.ReLU()
    )
  ...
  def forward(self, x):
    ...
    bekker_args = bekker_args / self.in_std
...
md = model.state_dict()
print_c_network(md, model.in_std, output_scaler)
