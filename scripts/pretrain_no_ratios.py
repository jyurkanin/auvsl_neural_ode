...
class TireNet(nn.Module):
  ...
  def compute_bekker_input_scaler(self, x):
    ...
    x_args = torch.cat((torch.abs(x[:,0])[:,None],
                        torch.abs(x[:,2])[:,None]), 1)
    
    self.in_x_mean = torch.mean(x_args, 0)
    temp = x_args - self.in_x_mean
    self.in_x_std = torch.sqrt(torch.var(temp, 0))
  ...
  def forward(self, x):
    ...
    x_args = torch.cat((torch.abs(x[:,0])[:,None],
                        torch.abs(x[:,2])[:,None]), 1)
    x_args = (x_args - self.in_x_mean) / self.in_x_std
    x_yhat = self.x_model.forward(x_args)
    
    fx = (yhat[:,0] * torch.tanh(1*diff)) + (x_yhat[:,0] * torch.tanh(-1*x[:,0]))
