...
class TireNet(nn.Module):
  ...
  def compute_bekker_input_scaler(self, x):
    ...
    self.input_scaler2 = StandardScaler()
    self.input_scaler2.fit(x[:,0:3])
  ...
  def forward(self, x):
    ...
    # yhat_all = yhat_sign_corrected
    # yhat_all[:,0] = yhat_sign_corrected[:,0] + yhat2[:,0]
    # yhat_all[:,1] = yhat_sign_corrected[:,1] + yhat2[:,1]
...
def get_evaluation_loss(test_x, test_y):
    ...
    print("Total Loss:", (np.mean(np.square(predicted_force.flatten() - test_y.flatten()))))
    print("Fx Loss: ", np.mean(np.square(predicted_force[:,0] - test_y[:,0])))
    print("Fy Loss: ", np.mean(np.square(predicted_force[:,1] - test_y[:,1])))
    print("Fz Loss: ", np.mean(np.square(predicted_force[:,2] - test_y[:,2])))
